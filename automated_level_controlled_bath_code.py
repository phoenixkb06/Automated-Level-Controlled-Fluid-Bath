...
import ssd1306      # OLED display driver (ssd1306.py)
...
while True:
    ...
    # 2.Check for a fresh button press to flip the memory state
    if btn_pump_now == 0 and btn_pump_prev == 1:
        pump_running = not pump_running
        
    btn_pump_prev = btn_pump_now
    
    # 3. Setpoint (Tap GP5 to increase target by 10%)
    ...
    # 4. Route power based on the memory state
    if pump_running: 
    ...
    # 6. Pace (keep at 0.1 for responsive button taps)
    time.sleep(0.1)
